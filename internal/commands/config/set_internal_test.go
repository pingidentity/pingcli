...
func Test_RunInternalConfigSet_InvalidProfileName(t *testing.T) {
	...
	expectedErrorPattern := `^failed to set configuration: invalid profile name: '.*'\. name must be lowercase and contain only alphanumeric characters, underscores, and dashes$`
	err := RunInternalConfigSet("noColor=true")
	testutils.CheckExpectedError(t, err, &expectedErrorPattern)
}
