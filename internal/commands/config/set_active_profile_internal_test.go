...
func Test_RunInternalConfigSetActiveProfile_InvalidProfileName(t *testing.T) {
	...
	expectedErrorPattern := `^failed to set active profile: invalid profile name: '.*'\. name must be lowercase and contain only alphanumeric characters, underscores, and dashes$`
