...
func Test_ResourceScopePingOneApi(t *testing.T) {
	clientInfo := testutils.GetClientInfo(t)

	tr := pingone_sso_testable_resources.ResourceScopePingOneApi(t, clientInfo)
	...
	expectedImportBlocks := []connector.ImportBlock{
		{
			...
			ResourceName: fmt.Sprintf("%s_%s", "PingOne API", tr.ResourceInfo.CreationInfo[testutils_resource.ENUM_NAME]),
			ResourceID:   fmt.Sprintf("%s/%s", clientInfo.PingOneExportEnvironmentID, tr.ResourceInfo.CreationInfo[testutils_resource.ENUM_ID]),
		},
	}

	// Existing scopes on the pingone api resource are generated. Test subset.
