...
func (r *PingOneSchemaAttributeResource) getSchemaData() (map[string]string, error) {
	...
	apiObjs, err := pingone.GetManagementAPIObjectsFromIterator[management.Schema](iter, "ReadAllSchemas", "GetSchemas", r.ResourceType())
	if err != nil {
		return nil, err
	}

	for _, schema := range apiObjs {
...
func (r *PingOneSchemaAttributeResource) getSchemaAttributeData(schemaId string) (map[string]string, error) {
	...
	apiObjs, err := pingone.GetManagementAPIObjectsFromIterator[management.EntityArrayEmbeddedAttributesInner](iter, "ReadAllSchemaAttributes", "GetAttributes", r.ResourceType())
	if err != nil {
		return nil, err
	}

	for _, innerObj := range apiObjs {
		if innerObj.SchemaAttribute != nil {
			schemaAttributeId, schemaAttributeIdOk := innerObj.SchemaAttribute.GetIdOk()
			schemaAttributeName, schemaAttributeNameOk := innerObj.SchemaAttribute.GetNameOk()
