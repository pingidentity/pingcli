# `pingcli authorize api-servers operations template`
...
```
pingcli authorize api-servers operations template [flags]
```
...
# Generate a template and save to a file
  pingcli authorize api-servers operations template > body.json

  # Edit the template, then create the API server operation
  pingcli authorize api-servers operations create --environment-id <env-id> --api-server-id <server-id> --from-file body.json
...
Example workflow:
  pingcli authorize api-servers operations template > body.json
  # edit body.json
  pingcli authorize api-servers operations create --from-file body.json
```
