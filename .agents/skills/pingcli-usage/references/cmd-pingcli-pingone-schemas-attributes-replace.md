# `pingcli pingone schemas attributes replace`
...
```
pingcli pingone schemas attributes replace [flags]
```
...
# Update a schema attribute from a JSON file (path flags are still required)
  pingcli pingone schemas attributes replace --environment-id <env-id> --schema-id <schema-id> --attribute-id <attribute-id> --from-file schema-attribute.json

  # Update a schema attribute from stdin
  pingcli pingone schemas attributes replace --environment-id <env-id> --schema-id <schema-id> --attribute-id <attribute-id> --from-file - < schema-attribute.json

  # Update from a JSON file, overriding the name and enabled state
  pingcli pingone schemas attributes replace --environment-id <env-id> --schema-id <schema-id> --attribute-id <attribute-id> --from-file schema-attribute.json --name renamed --enabled=false
```
