# `pingcli pingone users replace`
Update a user

## Synopsis

Update (replace) a user in a PingOne environment

```
pingcli pingone users replace [flags]
```

## Examples

```
# Replace a user from a JSON file
  pingcli pingone users replace --environment-id <env-id> --user-id <user-id> --from-file user.json

  # Replace a user from stdin
  pingcli pingone users replace --environment-id <env-id> --user-id <user-id> --from-file - < user.json

  # Replace a user from a JSON file, overriding fields with flags
  pingcli pingone users replace --environment-id <env-id> --user-id <user-id> --from-file user.json --nickname "Jamie" --population-id <pop-id>
```
