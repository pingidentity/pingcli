# `pingcli pingfederate sp idp-connections signing-settings replace`
Update IdP connection signing settings

## Synopsis

Update (replace) the signing settings singleton for a PingFederate SP IdP connection

```
pingcli pingfederate sp idp-connections signing-settings replace [flags]
```
...
# Replace the signing settings from a JSON file
  pingcli pingfederate sp idp-connections signing-settings replace --connection-id <connection-id> --from-file signing-settings.json
