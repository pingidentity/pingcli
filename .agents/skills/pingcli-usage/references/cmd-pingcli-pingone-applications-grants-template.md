# `pingcli pingone applications grants template`
...
```
pingcli pingone applications grants template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone applications grants template > application-grant.json

  # Edit the template (set resource.id and optionally scopes[].id), then create the grant
  pingcli pingone applications grants template --environment-id <env-id> --application-id <app-id> --from-file application-grant.json
...
Example workflow:
  pingcli pingone applications grants template > body.json
  # edit body.json
  pingcli pingone applications grants create --from-file body.json
```
...
- [`pingcli pingone applications grants`](cmd-pingcli-pingone-applications-grants.md) — Application Grants
