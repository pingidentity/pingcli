# `pingcli pingone credentials types replace`
...
```
pingcli pingone credentials types replace [flags]
```
...
# Replace a credential type from a JSON file
  pingcli pingone credentials types replace --environment-id <env-id> --credential-type-id <credential-type-id> --from-file credential-type.json

  # Replace a credential type from stdin
  pingcli pingone credentials types replace --environment-id <env-id> --credential-type-id <credential-type-id> --from-file - < credential-type.json

  # Replace a credential type, overriding scalar fields with flags
  # ("metadata" always comes from the file - it cannot be set as a flag)
  pingcli pingone credentials types replace --environment-id <env-id> --credential-type-id <credential-type-id> --title "Employee ID" --from-file credential-type.json
```
