# `pingcli pingfederate idp adapters template`
Generate an IDP adapter JSON template

## Synopsis

Generate a JSON skeleton template for IDP adapter create or replace bodies

```
pingcli pingfederate idp adapters template [flags]
```
...
# Generate a template and save to a file
  pingcli pingfederate idp adapters template > body.json

  # Edit the template, then create the IDP adapter
  pingcli pingfederate idp adapters create --from-file body.json
