# `pingcli pingfederate ping-one-connections replace`
Update a PingOne connection

## Synopsis

Update (replace) a PingFederate PingOne connection

```
pingcli pingfederate ping-one-connections replace [flags]
```

## Examples

```
# Update a PingOne connection from a JSON file (--id is still required)
