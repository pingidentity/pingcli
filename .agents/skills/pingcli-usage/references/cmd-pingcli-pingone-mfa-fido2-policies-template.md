# `pingcli pingone mfa fido2-policies template`
...
```
pingcli pingone mfa fido2-policies template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone mfa fido2-policies template > body.json

  # Edit the template, then create the FIDO2 policy
  pingcli pingone mfa fido2-policies create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone mfa fido2-policies template > body.json
  # edit body.json
  pingcli pingone mfa fido2-policies create --from-file body.json
```
