# `pingcli authorize api-servers operations apply`
...
```
pingcli authorize api-servers operations apply [flags]
```
...
# Create or update an API server operation (body supplies name and required paths)
  pingcli authorize api-servers operations apply --environment-id <env-id> --api-server-id <server-id> --from-file api-server-operation.json
