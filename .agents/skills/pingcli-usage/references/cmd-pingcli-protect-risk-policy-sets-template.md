# `pingcli protect risk-policy-sets template`
...
The template emits the risk policy set shape with all optional fields at their
zero values. Required fields are name and riskPolicies (an empty array is valid
for a catch-all set). Setting "default" to true promotes the policy set to the
environment default (only one policy set may be the default at a time — the API
automatically demotes any existing default when this flag is set to true).

Server-set read-only fields (id, createdAt, updatedAt, environment) are
excluded from the template.

```
pingcli protect risk-policy-sets template [flags]
```
...
# Generate a template and save to a file
  pingcli protect risk-policy-sets template > body.json
...
Example workflow:
  pingcli protect risk-policy-sets template > body.json
  # edit body.json
  pingcli protect risk-policy-sets create --from-file body.json
```
