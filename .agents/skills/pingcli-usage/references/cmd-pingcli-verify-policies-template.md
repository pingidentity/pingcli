# `pingcli verify policies template`
...
```
pingcli verify policies template [flags]
```
...
# Generate a template and save to a file
  pingcli verify policies template > body.json
...
Example workflow:
  pingcli verify policies template > body.json
  # edit body.json
  pingcli verify policies create --from-file body.json
```
