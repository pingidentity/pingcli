# `pingcli authorize api-servers operations replace`
...
```
pingcli authorize api-servers operations replace [flags]
```
...
# Update an API server operation from a JSON file (--environment-id, --api-server-id, and --api-server-operation-id are still required)
  pingcli authorize api-servers operations replace --environment-id <env-id> --api-server-id <server-id> --api-server-operation-id <operation-id> --from-file api-server-operation.json
  ...
  pingcli authorize api-servers operations replace --environment-id <env-id> --api-server-id <server-id> --api-server-operation-id <operation-id> --from-file api-server-operation.json --name "Read Widgets" --method GET,POST
```
