# `pingcli pingfederate idp sp-connections replace`
...
```
pingcli pingfederate idp sp-connections replace [flags]
```
...
# Update an SP connection from a JSON file (--id is still required)
  pingcli pingfederate idp sp-connections replace --id <id> --from-file sp-connection.json
