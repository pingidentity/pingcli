# `pingcli pingone sign-on-policies actions get`
...
```
pingcli pingone sign-on-policies actions get [flags]
```
...
```
# Read a specific sign-on policy action
  pingcli pingone sign-on-policies actions get --environment-id <env-id> --sign-on-policy-id <sop-id> --sign-on-policy-action-id <action-id>
```
