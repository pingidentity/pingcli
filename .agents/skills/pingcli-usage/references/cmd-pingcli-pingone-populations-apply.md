...
Create or update a population

## Synopsis

Idempotently create or update a population looked up by the "name" field in the JSON body within the supplied --environment-id. If no population with the given name exists it is created; if exactly one exists it is updated; if more than one exists the command fails.

```
pingcli pingone populations apply [flags]
```

## Examples

```
# Create or update a population (body supplies name and optional description)
  pingcli pingone populations apply --environment-id <env-id> --from-file population.json

  # Read body from stdin
  pingcli pingone populations apply --environment-id <env-id> --from-file - < population.json

  # Create or update from flags, without --from-file
  pingcli pingone populations apply --environment-id <env-id> --name "My Population" --description "Standard users"
```
