# `pingcli pingone davinci variables replace`
...
```
pingcli pingone davinci variables replace [flags]
```

## Examples

```
# Update a DaVinci variable from flags
  pingcli pingone davinci variables replace --environment-id <env-id> --variable-id <var-id> --name myVar --context company --data-type string --mutable=true

  # Update a DaVinci variable from a JSON file (--environment-id and --variable-id are still required)
  pingcli pingone davinci variables replace --environment-id <env-id> --variable-id <var-id> --from-file variable.json

  # Update a DaVinci variable from stdin
  pingcli pingone davinci variables replace --environment-id <env-id> --variable-id <var-id> --from-file - < variable.json
```
