# `pingcli pingone email-domains template`
Generate a trusted email domain JSON template

## Synopsis

Generate a JSON skeleton template for trusted email domain create bodies

```
pingcli pingone email-domains template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone email-domains template > body.json

  # Edit the template, then create the trusted email domain
  pingcli pingone email-domains create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone email-domains template > body.json
  # edit body.json
  pingcli pingone email-domains create --from-file body.json
```
