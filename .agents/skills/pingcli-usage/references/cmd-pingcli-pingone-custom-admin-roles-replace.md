# `pingcli pingone custom-admin-roles replace`
Update a custom admin role

## Synopsis

Update (replace) a custom admin role in a PingOne environment

```
pingcli pingone custom-admin-roles replace [flags]
```

## Examples

```
# Update a custom admin role from a JSON file (--environment-id and --custom-admin-role-id are still required)
