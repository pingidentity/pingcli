# `pingcli pingone webhooks apply`
Create or update a webhook

## Synopsis

Idempotently create or update a webhook looked up by the "name" field in the JSON body within the supplied --environment-id. If no webhook with the given name exists it is created; if exactly one exists it is updated; if more than one exists the command fails.

```
pingcli pingone webhooks apply [flags]
```
