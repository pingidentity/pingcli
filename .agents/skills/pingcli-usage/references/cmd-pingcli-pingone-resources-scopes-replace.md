# `pingcli pingone resources scopes replace`
Update a resource scope

## Synopsis

Update (replace) a resource scope in a PingOne environment

```
pingcli pingone resources scopes replace [flags]
```

## Examples

```
# Update a resource scope from a JSON file (--environment-id, --resource-id, and --resource-scope-id are still required)
