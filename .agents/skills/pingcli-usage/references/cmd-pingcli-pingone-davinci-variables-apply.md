# `pingcli pingone davinci variables apply`
Create or update a DaVinci variable

## Synopsis

Idempotently create or update a DaVinci variable looked up by the "name" field in the JSON body within the supplied --environment-id. If no variable with the given name exists it is created; if exactly one exists it is updated; if more than one exists the command fails.

```
pingcli pingone davinci variables apply [flags]
```
...
# Create or update a DaVinci variable from flags
  pingcli pingone davinci variables apply --environment-id <env-id> --name myVar --context company --data-type string --mutable=true

  # Create or update a DaVinci variable (body supplies name, context, dataType, etc.)
  pingcli pingone davinci variables apply --environment-id <env-id> --from-file variable.json
