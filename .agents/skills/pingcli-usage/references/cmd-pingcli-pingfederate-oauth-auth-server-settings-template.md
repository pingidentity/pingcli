# `pingcli pingfederate oauth auth-server-settings template`
Generate an oauth authorization server settings JSON template

## Synopsis

Generate a JSON skeleton template for oauth authorization server settings replace bodies

```
pingcli pingfederate oauth auth-server-settings template [flags]
```
...
# Generate a template and save to a file
  pingcli pingfederate oauth auth-server-settings template > body.json

  # Edit the template, then update the oauth authorization server settings
  pingcli pingfederate oauth auth-server-settings replace --from-file body.json
