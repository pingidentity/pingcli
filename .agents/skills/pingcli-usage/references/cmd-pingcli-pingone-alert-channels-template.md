# `pingcli pingone alert-channels template`
Generate an alert channel JSON template

## Synopsis

Generate a JSON skeleton template for alert channel create or replace bodies

```
pingcli pingone alert-channels template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone alert-channels template > body.json

  # Edit the template, then create the alert channel
  pingcli pingone alert-channels create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone alert-channels template > body.json
  # edit body.json
  pingcli pingone alert-channels create --from-file body.json
```
