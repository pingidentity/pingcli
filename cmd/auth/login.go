...
import (
	...
	auth_commands "github.com/pingidentity/pingcli/internal/commands/auth"
...
func authLoginRunE(cmd *cobra.Command, args []string) error {
	...
	if !flagProvided {
		...
		if err != nil || strings.TrimSpace(authType) == "" {
			...
			if err := auth_commands.RunInteractiveAuthConfig(os.Stdin); err != nil {
				return err
			}
	...
	if err := ensureAuthConfigurationExists(selectedMethod); err != nil {
		...
		if configErr := auth_commands.RunInteractiveAuthConfig(os.Stdin); configErr != nil {
			return configErr
		}
	}
...
func ensureAuthConfigurationExists(authMethod string) error {
	...
	case string(svcOAuth2.GrantTypeAuthCode):
		...
		return err
	default:
		return fmt.Errorf("%w: %s", ErrUnknownAuthMethod, authMethod)
	}
}
...
func performLoginByConfiguredType(ctx context.Context, authType, profileName string) error {
	...
	default:
		return &errs.PingCLIError{
			...
			Err:    auth_commands.ErrInvalidAuthType,
		}
	}
...
func handleLoginError(err error, ctx context.Context, profileName string) error {
	...
	if isMissingConfigError(err) {
		...
		reconfigure, promptErr := auth_commands.PromptForReconfiguration(os.Stdin)
		if promptErr != nil {
			return &errs.PingCLIError{
		...
		return &errs.PingCLIError{
			...
			Err:    auth_commands.ErrAuthConfigRequired,
		}
	}
