...
| `service.pingone.authentication.authorizationCode.redirectURI` | `--pingone-oidc-auth-code-redirect-uri` | PINGCLI_PINGONE_OIDC_AUTH_CODE_REDIRECT_URI | String | The redirect URI to use when using the auth code authorization grant type to authenticate to the PingOne management API.
