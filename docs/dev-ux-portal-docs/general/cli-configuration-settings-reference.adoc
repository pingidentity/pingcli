...
| `service.pingone.authentication.authCode.clientID` | `--pingone-oidc-auth-code-client-id` | PINGCLI_PINGONE_OIDC_AUTH_CODE_CLIENT_ID | String (UUID Format) | The auth code client ID used to authenticate to the PingOne management API.
| `service.pingone.authentication.authCode.environmentID` | `--pingone-oidc-auth-code-environment-id` | PINGCLI_PINGONE_OIDC_AUTH_CODE_ENVIRONMENT_ID | String (UUID Format) | The ID of the PingOne environment that contains the auth code client used to authenticate to the PingOne management API.
| `service.pingone.authentication.authCode.redirectURI` | `--pingone-oidc-auth-code-redirect-uri` | PINGCLI_PINGONE_OIDC_AUTH_CODE_REDIRECT_URI | String | The redirect URI to use when using the auth code authentication type to authenticate to the PingOne management API.
| `service.pingone.authentication.authCode.scopes` | `--pingone-oidc-auth-code-scopes` | PINGCLI_PINGONE_OIDC_AUTH_CODE_SCOPES | String Array | The auth code scope(s) used to authenticate to the PingOne management API.
