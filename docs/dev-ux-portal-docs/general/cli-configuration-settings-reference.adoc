...
| `login.storage.type` | `--storage` | PINGCLI_LOGIN_STORAGE_TYPE | String (Enum) | Auth token storage (default: secure_local)   secure_local  - Use OS keychain (default)   file_system   - Store tokens in ~/.pingcli/credentials   none          - Do not persist tokens
|===
