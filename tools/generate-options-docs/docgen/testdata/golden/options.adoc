...
| `auth.fileStorage` | `--file-storage` | PINGCLI_AUTH_FILE_STORAGE | Boolean | Store authentication tokens in file storage only, bypassing keychain. By default, keychain is attempted first with automatic fallback to file storage.
|===
