...
| `login.fileStorage` | `--file-storage` | PINGCLI_AUTH_STORAGE | Boolean | Store authentication tokens in local file storage only. Without this flag, keychain storage is attempted first with fallback to local file storage.
|===
